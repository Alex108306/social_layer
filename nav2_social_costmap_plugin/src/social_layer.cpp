         ...
         ++p_it)
    {
      ...
      for (int i = start_x; i < end_x; i++)
      {
        for (int j = start_y; j < end_y; j++)
        {
          ...
          {
            // PERSON STANDS STILL
            double sigma_when_still_x_=sigma_when_still_;
            double sigma_when_still_y_=sigma_when_still_ + 0.2;
            a = gaussian(x, y, cx, cy, amplitude_, sigma_when_still_x_,
                         sigma_when_still_y_, 0);
          }
          else
          {
